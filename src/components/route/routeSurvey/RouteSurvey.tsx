...
import { routeSurveySchema } from "@components/route/routeSurvey/_redux/model";
import type { RouteSurveyStateModel } from "@components/route/routeSurvey/_redux/routeSurveyReducer";
                        ...
                        formik.values.survey.map((question, qIdx) => {
                            ...
                            return (
                                <Box key={qIdx} sx={{ mb: 2, p: 2, border: '1px solid #eee', borderRadius: 2 }}>
                                            ...
                                            <FormControl key={oIdx} sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                                <input
                                                    ...
                                                    onChange={() => {
                                                        ...
                                                        void formik.setFieldValue('survey', updatedSurvey);
                                                    }}
                                                    style={{ marginRight: 8 }}
                                                />
                                    ...
                                    {questionError ? <Typography color="error" variant="body2" sx={{ mt: 1 }}>{questionError}</Typography> : null}
                                </Box>
                            );
                        })
                    ) : (
